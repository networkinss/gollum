...
**Test coverage — added (2026-09-17):** `backend_ollama_test.go` spins up an `httptest.Server` serving NDJSON `/api/generate` responses and covers: tokens arriving in order, `onToken` returning `false` after N tokens (asserts exactly N tokens received, and that the server observes the resulting context cancellation), `ctx` cancellation mid-stream (`errors.Is(err, context.Canceled)`), a server-sent `{"error": ...}` chunk surfacing as an error, and the existing non-streaming `Analyze` (previously untested — confirms it still sends `"stream": false`). `backend_embedded_test.go` is a `-tags llm`-gated compile-time assertion only (`var _ StreamingBackend = (*embeddedBackend)(nil)`) — it does **not** exercise real generation.

**Still open:** `backend_embedded.go`'s actual generation path (both `Analyze` and `AnalyzeStream`) has no runtime test — that needs `-tags llm`, a built `libbinding.a`, and a real (small) GGUF model, none of which this repo ships or should require for a normal `go test ./...`. If real coverage is wanted, gate it behind an env var pointing at a local model path (skip the test when unset) rather than making CI download a model.
